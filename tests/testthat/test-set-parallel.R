# ------------------------------------------------------------------------------
# iv_parallel_complement()

test_that("can parallel complement", {
  expect_identical(
    iv_parallel_complement(iv(1, 2), iv(5, 6)),
    iv(2, 5)
  )

  expect_identical(
    iv_parallel_complement(iv(1, 2), iv(-1, 0)),
    iv(0, 1)
  )
})
...
test_that("parallel complement of interval with itself is not allowed", {
  x <- iv(1, 2)
  expect_snapshot((expect_error(iv_parallel_complement(x, x))))
})
...
test_that("parallel complement of abutting intervals is not allowed", {
  ...
  y <- iv(0, 1)
  expect_snapshot((expect_error(iv_parallel_complement(x, y))))
...
test_that("parallel complement is generic over container", {
  ...
  expect_identical(iv_parallel_complement(x, y), nested_integer_iv(0, 1))
})
...
test_that("errors on gaps", {
  ...
  expect_snapshot((expect_error(iv_parallel_union(x, y))))

  expect_snapshot((expect_error(iv_parallel_union(y, x))))
})
...
test_that("parallel union is generic over container", {
  ...
  expect_identical(iv_parallel_union(x, y), nested_integer_iv(1, 3))
})

# ------------------------------------------------------------------------------
# iv_parallel_span()

test_that("can take the parallel span", {
  x <- iv(1, 3)
  y <- iv(2, 4)
...
test_that("merges across gaps", {
  ...
  expect_identical(iv_parallel_span(x, y), iv(1, 5))
  expect_identical(iv_parallel_span(y, x), iv(1, 5))
})

test_that("parallel span propagates NAs", {
  x <- iv(c(0, NA), c(2, NA))
  y <- iv(1, 4)
  ...
  expect_identical(
    iv_parallel_span(y, x),
    iv(c(0, NA), c(4, NA))
  )
})

test_that("parallel span is generic over container", {
  x <- nested_integer_iv(1, 2)
  y <- nested_integer_iv(2, 3)
  expect_identical(iv_parallel_span(x, y), nested_integer_iv(1, 3))
})

# ------------------------------------------------------------------------------
# iv_parallel_intersect()
...
test_that("parallel intersect is generic over container", {
  x <- nested_integer_iv(1, 3)
  y <- nested_integer_iv(2, 3)
  expect_identical(iv_parallel_intersect(x, y), nested_integer_iv(2, 3))
})

# ------------------------------------------------------------------------------
# iv_parallel_difference()
...
test_that("parallel difference between interval and itself is not allowed", {
  x <- iv(1, 3)
  expect_snapshot((expect_error(iv_parallel_difference(x, x))))
})
...
test_that("throws error when `y` is contained within `x`", {
  ...
  expect_snapshot((expect_error(iv_parallel_difference(x, y))))
})
...
test_that("parallel difference propagates NAs", {
  ...
  expect_identical(
    iv_parallel_difference(y, x),
    iv(c(2, NA), c(4, NA))
  )
})

# ------------------------------------------------------------------------------
# iv_parallel_symmetric_difference()

test_that("can symmetric parallel difference when LHS endpoint matches", {
  expect_identical(
    iv_parallel_symmetric_difference(iv(1, 3), iv(1, 5)),
    iv(3, 5)
  )

  expect_identical(
    iv_parallel_symmetric_difference(iv(1, 5), iv(1, 3)),
    iv(3, 5)
  )
})

test_that("can symmetric parallel difference when RHS endpoint matches", {
  expect_identical(
    iv_parallel_symmetric_difference(iv(1, 5), iv(3, 5)),
    iv(1, 3)
  )

  expect_identical(
    iv_parallel_symmetric_difference(iv(3, 5), iv(1, 5)),
    iv(1, 3)
  )
})
...
test_that("symmetric parallel difference propagates NAs", {
  x <- iv(c(0, NA), c(4, NA))
  y <- iv(1, 4)
  ...
  expect_identical(
    iv_parallel_symmetric_difference(y, x),
    iv(c(0, NA), c(1, NA))
  )
})
