...
iv is a package dedicated to working with intervals in a generic way.
...
The only interval type that is supported by iv is a *right-open* interval, i.e. `[a, b)`.
...
In iv, it is required that `start < end` when generating an interval vector.
...
One of the most compelling reasons to use iv is that it tries to make finding overlapping intervals as easy as possible.
...
Grouping overlapping intervals is often a useful way to create a new variable to group on with `dplyr::group_by()`.
...
Splitting an iv into its disjoint pieces is another operation that works nicely with `group_by()`.
...
`iv_complement()` works on a single iv, while all the others work on two intervals at a time.
...
One case in particular comes from combining `iv_locate_overlaps()` with `iv_pairwise_intersect()`.
...
For example, `iv_pairwise_intersect()` requires that `x[i]` and `y[i]` overlap, otherwise they would result in an empty interval, which isn't allowed.
...
-   Set-like operations treat missing intervals as *overlapping* other missing intervals, but they won't overlap any other interval.
    These include `iv_locate_overlaps()`, `iv_intersect()`, and `iv_splits()`.
