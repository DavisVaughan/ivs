...
But how can we sum up the costs? For this, we need to turn to `iv_identify_group()`, which allows us to replace each interval in the original `range` column with the merged interval that it maps to. This will give us something to group on so we can sum up the costs.
